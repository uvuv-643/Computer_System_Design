...
int main(void)
{
  ...
  while (1)
  {
	...
	switch (state) {
	 case GREEN:
		 if (pin_info == 1) switch_to_button(GREEN_SHORT);
		 else if (ready_to_switch()) switch_to_button_with_timer_flush(GREEN_FLUSHING);
		 break;
	 case GREEN_FLUSHING:
		 if (pin_info == 1) switch_to_button(GREEN_FLUSHING_SHORT);
		 else if (ready_to_switch()) switch_to_button_with_timer_flush(YELLOW);
		 break;
	 case RED:
		 if (pin_info == 1) switch_to_button(RED_SHORT);
		 else if (ready_to_switch()) switch_to_button_with_timer_flush(GREEN);
		 break;
	 case YELLOW:
		 if (pin_info == 1) switch_to_button(YELLOW_SHORT);
